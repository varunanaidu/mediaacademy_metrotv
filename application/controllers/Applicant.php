...
class Applicant extends SITE_Controller {
	...
	function index() {
		...
		$data['vacancy'] = $this->sitemodel->view('tr_vacant', '*');

		$data['applicant_apply'] = $this->sitemodel->view("tr_vacant tv", "tv.vacant_title as a", array("tc.candidat_id"=>$this->log_user), array("tr_applicant ta"=>"ta.vacant_id=tv.vacant_id,", "tab_candidat tc"=>"tc.candidat_id=ta.candidat_id,") );

		$data['university'] = $this->sitemodel->view('tab_university', '*');
		
		$data['city'] = $this->sitemodel->view('tab_kota', '*');
		...
		# LOAD F5 EMPLOYMENT 
		$data['applicant_employment'] = $this->sitemodel->view('candidat_work_exp', '*', ['candidat_id' => $this->log_user], false, 'work_exp_to DESC');
	...
	{
		$msg['type']		= 'done';
		$msg['education'] 	= $this->sitemodel->view('candidat_edu', '*', ['candidat_id' => $this->log_user]);
		$msg['work'] 		= $this->sitemodel->view('candidat_work_exp', '*', ['candidat_id' => $this->log_user], false, 'work_exp_to DESC');

		echo json_encode($msg);
	}

	function update_form_applicant()
	{
		...
		$allowed_size			= 200;
		$fileCount 				= count($_FILES['uploadfile']['type']);
		$type 					= "";

		for ($i = 0; $i < $fileCount; $i++) {

			if ($_FILES['uploadfile']['error'][$i] != 4) {
				if (empty($_FILES['uploadfile'][$i]) and $_FILES['uploadfile']['size'][$i] < 0) {
				...
				if (in_array($ext, $allowed_ext['cv'])) {
					$type = 'cv';
				}else{
					$type = 'photo';
				}

				if ($_FILES['uploadfile']['size'][$i] > $allowed_size*1024) {
				...
				$hasMoved 		= move_uploaded_file($_FILES['uploadfile']['tmp_name'][$i], $target_dir.$new_filename);

				if ($hasMoved) {
					if ($type == 'cv') {
						$this->sitemodel->update('tab_candidat', ['candidat_cv' => $new_filename], ['candidat_id' => $this->log_user]);
					}
					else if ($type == 'photo') {
						$this->sitemodel->update('tab_candidat', ['candidat_foto' => $new_filename], ['candidat_id' => $this->log_user]);
					}
				}
			}
		...
		$this->form_validation->set_rules('idnum','ID Number','trim|required');
		$this->form_validation->set_rules('status', 'Martial Status', 'trim|required');

		$this->form_validation->set_rules('dom','Date Of Marriage','trim');
		$this->form_validation->set_rules('spousename','Spouse Name','trim');
		$this->form_validation->set_rules('sdob','Spouse DOB','trim');
		...
		$this->form_validation->set_rules('ca_city','Current Address - City','trim|required');
		$this->form_validation->set_rules('curr_district','Current Address - District','trim|required');
		$this->form_validation->set_rules('ca_zip_code','Current Address - Postal Code','trim|required|max_length[10]');
		$this->form_validation->set_rules('ca_ph','Current Address - Phone','trim|required');
		$this->form_validation->set_rules('per_address','Permanent Address','trim|required');
		$this->form_validation->set_rules('pa_city','Permanent Address - City','trim|required');
		$this->form_validation->set_rules('per_district','Permanent Address -District','trim|required');
		$this->form_validation->set_rules('pa_zip_code','Permanent Address - Postal Code','trim|required|max_length[10]');
		$this->form_validation->set_rules('pa_ph','Permanent Address - Phone','trim|required');
		$this->form_validation->set_rules('candidat_phone', 'Phone Number', 'trim|required');
		$this->form_validation->set_rules('email', 'Candidate Email', 'required');
		$this->form_validation->set_rules('candidat_whatsapp', 'Whatsapp Number', 'trim');
		$this->form_validation->set_rules('candidat_instagram', 'Instagram', 'trim');
		$this->form_validation->set_rules('candidat_facebook', 'Facebook', 'trim');
		$this->form_validation->set_rules('candidat_linkedin', 'Linked In', 'trim');

		$this->form_validation->set_rules('describe','Describe Yourself','trim|required');
		$this->form_validation->set_rules('salary','Expected Salary','trim|required');
		...
		$applicant_data = [
			'candidat_email' 	=>	$this->input->post('email'),
			'candidat_name'		=>	$this->input->post('fullname'),
			'candidat_phone'	=>	$this->input->post('candidat_phone'),
			'curr_address'		=>	$this->input->post('curr_address'),
			'curr_district'		=>	$this->input->post('curr_district'),
			'ca_city'			=>	$this->input->post('ca_city'),
			'ca_zip_code'		=>	$this->input->post('ca_zip_code'),
			'ca_ph'				=>	$this->input->post('ca_ph'),
			'per_address'		=>	$this->input->post('per_address'),
			'per_district'		=>	$this->input->post('per_district'),
			'pa_city'			=>	$this->input->post('pa_city'),
			'pa_zip_code'		=>	$this->input->post('pa_zip_code'),
			'pa_ph'				=>	$this->input->post('pa_ph'),
			...
			'expect_salary'		=>	$this->input->post('salary'),
			'candidat_describe'	=>	$this->input->post('describe'),
		];

		if ($this->input->post('status') == 'Married') {
			$applicant_data['marital_date'] 		= $this->input->post('dom');
			$applicant_data['spouse_name']			= $this->input->post('spousename');
			$applicant_data['spouse_dob']			= $this->input->post('sdob');
			$applicant_data['spouse_occupation']	= $this->input->post('spouse_occupation');
		}

		if ($this->input->post('work_bg') == 'Yes') {
			$applicant_data['candidat_latest_position'] = $this->input->post('latestPost');
		}
		...
		// FAMILY
		for ($i = 0; $i < count($this->input->post('applicant_family[family_id]')); $i++) {

			if ( !empty($this->input->post('applicant_family[family_name]')[$i]) && !empty($this->input->post('applicant_family[family_dob]')[$i]) && !empty($this->input->post('applicant_family[family_gender]')[$i]) && !empty($this->input->post('applicant_family[family_edu]')[$i]) ) {

				if ($this->input->post('applicant_family[family_relation]')[$i] == 'CHILD') {

					$child_id = $this->input->post('applicant_family[family_id]')[$i];
					
					$applicant_children = [
						'child_name'	=> $this->input->post('applicant_family[family_name]')[$i],
						'child_dob'		=> $this->input->post('applicant_family[family_dob]')[$i],
						'child_gender'	=> $this->input->post('applicant_family[family_gender]')[$i],
						'child_edu'		=> $this->input->post('applicant_family[family_edu]')[$i],
					];

					if (empty($child_id)) {
						$applicant_children['candidat_id']	=	$this->log_user;
						$this->sitemodel->insert('candidat_children', $applicant_children);
					}else{
						$this->sitemodel->update('candidat_children', $applicant_children, ['candidat_id' => $this->log_user, 'child_id' => $child_id ]);
					}

				}else{

					$family_id = $this->input->post('applicant_family[family_id]')[$i];
					
					$applicant_family = [
						'family_name'		=> $this->input->post('applicant_family[family_name]')[$i],
						'family_relation'	=> $this->input->post('applicant_family[family_relation]')[$i],
						'family_dob'		=> $this->input->post('applicant_family[family_dob]')[$i],
						'family_gender'		=> $this->input->post('applicant_family[family_gender]')[$i],
						'family_edu'		=> $this->input->post('applicant_family[family_edu]')[$i],
					];

					if (empty($family_id)) {
						$applicant_family['candidat_id']	=	$this->log_user;
						$this->sitemodel->insert('candidat_family', $applicant_family);
					}else{
						$this->sitemodel->update('candidat_family', $applicant_family, ['candidat_id' => $this->log_user, 'family_id' => $family_id ]);
					}

				}

			}else{
				$msg['msg'] = "Family data is not complete";
				echo json_encode($msg);
				return;
			}
		...
		for ($i = 0; $i < count($this->input->post('applicant_employment[work_exp_id]')); $i++) {

			if ( !empty($this->input->post('applicant_employment[company_name]')[$i]) ) {
				$work_exp_id = $this->input->post('applicant_employment[work_exp_id]')[$i];

				$applicant_employment = [
					'company_name'		=> $this->input->post('applicant_employment[company_name]')[$i],
					'work_exp_title'	=> $this->input->post('applicant_employment[work_exp_title]')[$i],
					'last_salary'		=> $this->input->post('applicant_employment[last_salary]')[$i],
					'job_desc'			=> $this->input->post('applicant_employment[job_desc]')[$i],
					'work_exp_from'		=> $this->input->post('applicant_employment[work_exp_from]')[$i],
					'work_exp_to'		=> $this->input->post('applicant_employment[work_exp_to]')[$i],
				];

				if (empty($work_exp_id)) {
					$applicant_employment['candidat_id']	=	$this->log_user;
				// echo json_encode($applicant_employment);
					$this->sitemodel->insert('candidat_work_exp', $applicant_employment);
				}else{
				// echo json_encode($applicant_employment);
					$this->sitemodel->update('candidat_work_exp', $applicant_employment, ['candidat_id' => $this->log_user, 'work_exp_id' => $work_exp_id ]);
				}
			}
		...
		for ($i = 0; $i < count($this->input->post('applicant_edu[cedu_id]')); $i++) {

			if ( !empty($this->input->post('applicant_edu[edu_institute]')[$i]) ) {
				$cedu_id = $this->input->post('applicant_edu[cedu_id]')[$i];

				$applicant_edu = [
					'edu_title'		=> $this->input->post('applicant_edu[edu_title]')[$i],
					'edu_institute'		=> $this->input->post('applicant_edu[edu_institute]')[$i],
					'edu_major'			=> $this->input->post('applicant_edu[edu_major]')[$i],
					'edu_start'			=> $this->input->post('applicant_edu[edu_start]')[$i],
					'edu_end'			=> $this->input->post('applicant_edu[edu_end]')[$i],
					'gpa'				=> $this->input->post('applicant_edu[gpa]')[$i],
				];

				if (empty($cedu_id)) {
					$applicant_edu['candidat_id']	=	$this->log_user;
				// echo json_encode($applicant_edu);
					$this->sitemodel->insert('candidat_edu', $applicant_edu);
				}else{
				// echo json_encode($applicant_edu);
					$this->sitemodel->update('candidat_edu', $applicant_edu, ['candidat_id' => $this->log_user, 'cedu_id' => $cedu_id ]);
				}
			}else{
				$msg['msg'] = "Educational Background data is not complete";
				echo json_encode($msg);
				return;
			}
		...
		for ($i = 0; $i < count($this->input->post('applicant_organization[org_id]')); $i++) {

			if ( !empty($this->input->post('applicant_organization[activities]')[$i]) ) {

				$org_id = $this->input->post('applicant_organization[org_id]')[$i];

				$applicant_organization = [
					'activities'		=> $this->input->post('applicant_organization[activities]')[$i],
					'org_pos'	=> $this->input->post('applicant_organization[org_pos]')[$i],
				];

				if (empty($org_id)) {
					$applicant_organization['candidat_id']	=	$this->log_user;
				// echo json_encode($applicant_organization);
					$this->sitemodel->insert('candidat_organizational', $applicant_organization);
				}else{
				// echo json_encode($applicant_organization);
					$this->sitemodel->update('candidat_organizational', $applicant_organization, ['candidat_id' => $this->log_user, 'org_id' => $org_id ]);
				}
		...
		############################################ PERLU REVISI
			...
			$msg['type'] = 'done';
			$msg['msg'] = 'Successfully change your data';
			echo json_encode($msg);
		}
	...
	function updateCandidat()
	{
		...
		$applicant_data = array(
			...
			'pob'					=>	$this->input->post('pob'),
			'dob'					=>	$this->input->post('dob'),
			'candidat_instagram'	=>	$this->input->post('instagram'),
			'candidat_facebook'		=>	$this->input->post('facebook'),
			'candidat_linkedin'		=>	$this->input->post('linkedin'),
		);
		...
		$applicant_edu = [
			...
			'edu_institute'		=> $this->input->post('edu_institute'),
			'edu_major'			=> $this->input->post('edu_major'),
			'edu_start'			=> $this->input->post('edu_start'),
			'edu_end'			=> $this->input->post('edu_end'),
			'gpa'				=> $this->input->post('edu_gpa'),
		];

		if ($this->input->post('cedu_id') == '') {
			$applicant_edu['candidat_id']	=	$this->log_user;
			$this->sitemodel->insert('candidat_edu', $applicant_edu);
		}else{
			$this->sitemodel->update('candidat_edu', $applicant_edu, ['candidat_id' => $this->log_user, 'cedu_id' => $this->input->post('cedu_id') ]);
		}

		$applicant_employment = [
			'company_name'		=> $this->input->post('company_name'),
			'work_exp_title'	=> $this->input->post('work_exp_title'),
			'last_salary'		=> $this->input->post('last_salary'),
			'job_desc'			=> $this->input->post('job_desc'),
			'work_exp_from'		=> $this->input->post('work_exp_from'),
			'work_exp_to'		=> $this->input->post('work_exp_to'),
		];

		if ($this->input->post('work_exp_id') == '') {
			$applicant_employment['candidat_id']	=	$this->log_user;
			$this->sitemodel->insert('candidat_work_exp', $applicant_employment);
		}else{
			$this->sitemodel->update('candidat_work_exp', $applicant_employment, ['candidat_id' => $this->log_user, 'work_exp_id' => $this->input->post('work_exp_id') ]);
		}
